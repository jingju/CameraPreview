...
void MVRecordingPreviewController::prepareEGLContext(ANativeWindow* window, JavaVM *g_jvm, jobject obj, int screenWidth, int screenHeight, int cameraFacingId){
    ...
    pthread_create(&_threadId, 0, threadStartCallback, this);//创建线程
}
...
void MVRecordingPreviewController::processMessage() {
    ...
    while (renderingEnabled) {//todo 循环取消息
        Message* msg = NULL;
...
bool MVRecordingPreviewController::initialize() {
    ...
    this->buildRenderInstance();//创建渲染引擎
    this->configCamera();
...
/**
 * 创建窗口，并将相关的参数和context关联起来
 */
void MVRecordingPreviewController::createPreviewSurface() {
    previewSurface = eglCore->creatWindowSurface(_window);
...
void MVRecordingPreviewController::configCamera() {
    ...
    jclass jcls = env->GetObjectClass(obj);
    if (NULL != jcls) {
        jmethodID configCameraCallback = env->GetMethodID(jcls, "configCameraFromNative", "(I)Lcom/jingju/camerapreview/preview/CameraConfigInfo;");
