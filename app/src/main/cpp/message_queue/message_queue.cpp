...
void MessageQueue::init() {
    int initLockCode = pthread_mutex_init(&mLock, NULL);//初始化锁
    int initConditionCode = pthread_cond_init(&mCondition, NULL);//初始化锁的条件变量
    mNbPackets = 0;
    mFirst = NULL;
    mLast = NULL;
    mAbortRequest = false;
 ...
 * 从而避免了线程不断轮询检查该条件是否成立而降低效率的情况，这是实现了效率提高。。。
 * @param msg
 * @param block
 * @return
 */
